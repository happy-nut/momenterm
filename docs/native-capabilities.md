# Native Review Capabilities (Complete)

> Status: **complete**. The port-to-native effort is done — every item
> below is a shipped, verified capability. This file is kept as
...
Momenterm is a native macOS review workbench. The goal was a complete local review loop delivered entirely in native Swift, without any Electron, Node, or bundled JS runtime artifacts.
...
- Implement keyboard navigation: F7 and Shift+F7 move through review targets; Cmd+0, Cmd+1, Cmd+9, Cmd+Down, Cmd+A, double-Shift, PageUp/PageDown, sidebar arrows, comment-card arrows, `e`, Backspace, Escape, merged Opt+Enter/Opt+Arrow, history arrows/Enter, and terminal input keys are all covered by `native-guard-smoke`.
- Implement source review: source view supports line gutters, raw/rendered toggle, Markdown rows with sanitized embedded HTML, CSV rows, blank-line caret support, syntax highlighting, and shared comments across diff/source.
...
- Implement the integrated terminal: native PTY is the default base workbench, review tools float above it, terminal sessions open as tabs from `~` by default, tab focus/rename/close are native, send-to-terminal handoff works, and npm/Node/Electron launch environment is stripped.
...
- Honor the native-core review contracts: staged changes are included in normal review mode, rename detection is enabled, large or binary untracked files use binary-style diff markers, source metadata includes language/change/signature fields, file state signatures match the review payload shape, and HTTP client environments are collected natively.
- Add an A/B source-collection smoke: `scripts/ab-smoke.sh` runs Momenterm's native core against fixture Git repositories and checks review counts, review signatures, update keys, file states, source metadata, lazy source data, HTTP environments, lazy body counts, app shell markers, and Darcula markers across modified, staged, untracked, rename/delete/image, and private-env cases.
...
rg -n "NODE_BIN|libnode|buildDiffReview|renderWelcomeHtml|performHttpRequest|Support/" Sources/Momenterm scripts Package.swift README.md -g '!*.app' -g '!ab-smoke.mjs'
```

The core smoke test checks native review markers in generated HTML. The native-guard smoke boots the embedded client script in a VM, calls the real syntax highlighter, and verifies each shortcut, settings surface, and Darcula visual token one by one. The A/B smoke exercises Momenterm's native output for representative Git states. The runtime scan rejects known JS-runtime markers in the shipped native app sources and scripts. The PTY smoke test runs with polluted npm/Node locale environment variables and verifies the spawned shell receives a clean UTF-8 environment.
