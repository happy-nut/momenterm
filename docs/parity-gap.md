...
The core smoke test checks native parity markers in generated HTML. The parity smoke boots the embedded client script in a VM, calls the real syntax highlighter, and verifies each shortcut, settings surface, and Darcula visual token one by one. The runtime scan rejects known Monacori runtime markers in the shipped native app sources and scripts. The PTY smoke test runs with polluted npm/Node locale environment variables and verifies the spawned shell receives a clean UTF-8 environment.
