...
- Implement integrated terminal parity: native PTY is the default base workbench, review tools float above it, terminal sessions open as tabs from `~` by default, tab focus/rename/close are native, send-to-terminal handoff works, and npm/Node/Electron launch environment is stripped.
